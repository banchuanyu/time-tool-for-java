...
2. 参照 [参数说明](#`application.properties` 参数说明) 修改 `shepher-web/src/main/resources/application-dev.properties` 的参数配置
3. 运行脚本

    ```sh
    $sh script/dev-build-start.sh
    ```
4. 在浏览器中访问 `http://localhost:8089` 或自定义的服务地址（参照 [参数说明](#`application.properties` 参数说明)）
...
2. 参照 [参数说明](#`application.properties` 参数说明) 修改 `shepher-web/src/main/resources/application-docker.properties` 的参数配置
